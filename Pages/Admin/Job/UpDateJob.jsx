...
import { SortJobs, upDataJob } from '../../../Redux/reducer/Jobs'
...
function UpDateJob() {
  ...
  const {companyName,
    ...
    counterApplications,
    image
  } = useSelector(state => state.Jobs.Jobs).find(job => job.id == id)
    {
        ...
        const formik = useFormik({
          ...
          onSubmit: values => {
            if (inputImg.current.files[0]) {
              dispatch(upDataJob({ ...values, Status, image: URL.createObjectURL(inputImg.current.files[0]), counterApplications, Applications, posted: new Date().toUTCString() ,id}))
              
            } else {
              
              dispatch(upDataJob({ ...values, Status, image, counterApplications, Applications, Posted: new Date().toUTCString(),id }))
            }
            dispatch(SortJobs())
            Navigate(-1)
          },
                      ...
                      <div>
                        ...
                        <div className="row justify-content-between">
                          <div className="row col-lg-7 ms-1 ms-md-0">
                            ...
                            <div className="form-outline col-lg-6 col-12 my-2">
                              <label className="text-light h4" ></label>
                              <select
                                style={{ cursor: "pointer" }}
                                className="w-100 me-5 p-2"
                                id="location"
                        ...
                        <div className="Skills d-flex flex-wrap p-2">
                          ...
                          {allSkills.map((skill) => <button key={skill}
                          ...
                          <input
                            ...
                            type="text"
                            onKeyUp={(e) => e.key == 'Enter' && (allSkills.some((skill) => skill == e.target.value) && e.target.value == "" ? "" : setAllSkills([...allSkills, e.target.value]))}
                          />
                        </div>
